...
+ Owner root identifier, or `unowned` for legacy rows with no owner
+ List of up to 3 unique identifiers as numbered 'fixed connections'
...
and all of its parents. The owner must be a fully available Kennedy, user, or
group root.
...
UpdateNode allows Kennedy to modify the owner, short name, short description,
and long description of a node. A displayed `unowned` node must receive an
owner when it is updated.
...
SetFixedConnection allows Kennedy to assign any node into one of three numbered
fixed slots. If a node already exists in that slot, it is replaced. Fixed
connections are arbitrary and do not represent tasks, priority, or completion.
